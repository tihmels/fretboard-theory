...
All rendering is driven by `annotateGrid`, which composes a `FretboardNote[][]` grid with an `AnnotationContext` (root, scale, chord, labelMode, fretRange) into a flat `NoteAnnotation[][]` that React SVG components render.
